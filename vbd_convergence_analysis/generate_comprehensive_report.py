...
<div class="container">
    ...
    <div class="summary" id="executive-summary">
        ...
        <h3>Trajectory-Based Comparison (same starting states)</h3>
        <p>All methods evaluated from the <em>same</em> reference trajectory snapshots, eliminating
        the confound of compounded inter-substep drift.</p>
        <table class="comparison-table" style="width:auto;">
        <tr><th>Method</th><th>Iter 0</th><th>Iter 9</th><th>Overall Ratio</th><th>Per-iter Geo Mean</th></tr>
        <tr><td>Baseline (alpha=1.0)</td><td>0.41</td><td>0.48</td>
            <td style="color:#c62828;font-weight:bold;">1.16 (diverges)</td><td>1.020</td></tr>
        <tr><td>Alpha=0.9</td><td>0.41</td><td>0.27</td>
            <td style="color:#2e7d32;font-weight:bold;">0.66 (converges)</td><td>0.954</td></tr>
        <tr><td>Alpha=0.7</td><td>0.43</td><td>0.27</td>
            <td style="color:#2e7d32;font-weight:bold;">0.63 (converges)</td><td>0.950</td></tr>
        <tr><td>Alpha=0.5</td><td>0.64</td><td>0.55</td>
            <td style="color:#558b2f;">0.87 (slow)</td><td>0.985</td></tr>
        </table>

        <ul>
            <li><strong>Baseline GS diverges</strong> (ratio {bl_m['median_ratio']:.4f}).
                Cross-color interference causes the force residual to <em>increase</em> with each iteration.
                Iterations 2&ndash;10 make the solution worse, not better.</li>
            <li><strong>Alpha=0.7&ndash;0.9 genuinely converge</strong> with ~35% residual reduction over 10 iterations
                (ratio 0.63&ndash;0.66). Most improvement comes in iterations 0&ndash;2, then convergence stalls
                (per-iteration ratio &rarr; 1.0 by iteration 5).</li>
            <li><strong>Alpha=0.5 is over-damped</strong> &mdash; takes too small a step. The first iteration actually
                lands further from the minimum than baseline (starting residual 0.64 vs 0.41).
                It slowly recovers (ratio 0.87) but is less effective than alpha=0.7&ndash;0.9.</li>
            <li><strong>This compounds across timesteps</strong>: baseline's per-substep divergence accumulates over
                a simulation, while alpha=0.7 converges each substep. After 100 substeps, baseline's
                accumulated residual is ~100x higher. This explains the large absolute gaps in the
                convergence data tables below (which do <em>not</em> use controlled starting states).</li>
            <li><strong>Jacobi mode</strong> (ratio {jacobi_m['median_ratio']:.4f}) shows steady per-iteration convergence
                (no cross-color interference), but starts from much higher residual.</li>
            <li>Self-contact (ratio {sc_m['median_ratio']:.4f}) does not degrade convergence vs baseline.</li>
            <li><strong>Hessian verification</strong>: Neo-Hookean uses an intentional SPD projection.
                Bending uses Gauss-Newton approximation (PSD by construction). Both are correct by design.</li>
        </ul>
    </div>
