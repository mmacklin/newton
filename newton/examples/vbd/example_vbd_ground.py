...
"""AVBD 3D Ground scene recreated with Newton VBD."""
...
TITLE = "Ground"
DESCRIPTION = "Single dynamic cube falling onto a static box ground."
REPORT_FRAMES = 180
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.shadow_distance = 14.0
        self.viewer.set_camera(pos=wp.vec3(9.30248234, -9.30248234, 6.78828201), pitch=-20.0, yaw=135.0)
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "fov"):
            self.viewer.camera.fov = 55.0
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "look_at"):
            self.viewer.camera.look_at(wp.vec3(0.0, 0.0, 2.0))
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        box = builder.add_body(xform=_xform((0.0, 0.0, 4.0)), label="box")
        builder.add_shape_box(box, hx=0.5, hy=0.5, hz=0.5, cfg=self._shape_cfg(1.0, 0.5), label="box_shape")
