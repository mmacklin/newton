...
class Example:
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        size = 16
        self.add_box(builder, (100.0, 100.0, 1.0), 0.0, 0.5, (0.0, 0.0, -0.5), label="ground")
        for y in range(size):
    ...
    def _validate_scene(self) -> None:
        ...
        initial_pos = np.asarray([self.initial_body_positions[body] for body in brick_indices], dtype=np.float64)
        ...
        self.assert_final_speed_below(5.0)
        self.assert_final_angular_speed_below(8.0)
        if observed_abs_y > PYRAMID_PLANE_ABS_Y_LIMIT:
            raise AssertionError(
                f"{self.title}: brick centers left the source y=0 plane by {observed_abs_y:.3f} m; "
                f"expected <= {PYRAMID_PLANE_ABS_Y_LIMIT:.3f} m"
            )
        if observed_lateral_span > PYRAMID_PLANE_SPAN_LIMIT:
            raise AssertionError(
                f"{self.title}: brick y-span reached {observed_lateral_span:.3f} m; "
        ...
        if brick_final_max_z < 6.0:
            raise AssertionError(
                f"{self.title}: tallest brick center is {brick_final_max_z:.3f} m; "
