...
class Example:
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        for i in range(10):
            self.add_box(builder, (1.0, 1.0, 1.0), 1.0, 0.5, (0.0, 0.0, i * 1.5 + 1.0), label=f"stack_{i}")
    ...
    def _validate_scene(self) -> None:
        ...
        self.assert_final_speed_below(0.25)
        self.assert_final_angular_speed_below(0.5)
        if observed_xy_radius > STACK_CENTER_RADIUS_LIMIT:
            raise AssertionError(
                f"{self.title}: stack center radius reached {observed_xy_radius:.3f} m; "
                f"expected <= {STACK_CENTER_RADIUS_LIMIT:.3f} m"
            )
        if observed_xy_spread > STACK_XY_SPREAD_LIMIT:
            raise AssertionError(
                f"{self.title}: stack xy spread reached {observed_xy_spread:.3f} m; "
                f"expected <= {STACK_XY_SPREAD_LIMIT:.3f} m"
            )
        if observed_line_fit > STACK_LINE_FIT_LIMIT:
            raise AssertionError(
                f"{self.title}: stack centers deviated {observed_line_fit:.3f} m from a fitted line; "
                f"expected <= {STACK_LINE_FIT_LIMIT:.3f} m"
            )
        if observed_tilt > STACK_TILT_LIMIT:
            raise AssertionError(
                f"{self.title}: stack fitted line tilt reached {observed_tilt:.3f} xy/z; "
