...
"""AVBD 3D Bridge scene recreated with Newton VBD."""
...
TITLE = "Bridge"
DESCRIPTION = "Forty planks coupled by paired hard ball joints and loaded by falling cubes."
REPORT_FRAMES = 150
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.shadow_distance = 50.0
        self.viewer.set_camera(pos=wp.vec3(-0.00000000, -47.55282581, 25.45084972), pitch=-18.0, yaw=90.0)
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "fov"):
            self.viewer.camera.fov = 55.0
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "look_at"):
            self.viewer.camera.look_at(wp.vec3(0.0, 0.0, 10.0))
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        for i in range(40):
            body = builder.add_body(
                xform=_xform((float(i) - 20.0, 0.0, 10.0)),
                is_kinematic=i == 0 or i == 39,
                label=f"plank_{i}",
            )
            builder.add_shape_box(
                ...
                hy=2.0,
                hz=0.25,
                cfg=self._shape_cfg(0.0 if i == 0 or i == 39 else 1.0, 0.5),
                label=f"plank_{i}_shape",
            )
            if prev >= 0:
                builder.add_joint_ball(
                    parent=prev,
                    child=body,
                    parent_xform=_xform((0.5, 2.0, 0.0)),
                    child_xform=_xform((-0.5, 2.0, 0.0)),
                    collision_filter_parent=True,
                    label=f"bridge_joint_a_{i}",
                )
                builder.add_joint_ball(
                    parent=prev,
                    child=body,
        ...
        for x in range(10):
            for y in range(5):
                body = builder.add_body(
                    xform=_xform((float(x) - 5.0, 0.0, float(y) + 12.0)), label=f"bridge_load_{x}_{y}"
                )
                builder.add_shape_box(
                    body, hx=0.5, hy=0.5, hz=0.5, cfg=self._shape_cfg(1.0, 0.5), label=f"bridge_load_{x}_{y}_shape"
                )
    ...
    def test_final(self) -> None:
        ...
        if self.final_speed > 12.0:
            raise AssertionError(f"{self.title}: final speed {self.final_speed:.3f} exceeds 12.000")
        ...
        if self.final_max_abs_position > 24.0:
            raise AssertionError(f"{self.title}: final max |position| {self.final_max_abs_position:.3f} exceeds 24.000")
