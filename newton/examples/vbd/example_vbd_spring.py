...
"""AVBD 3D Spring scene recreated with Newton VBD."""
...
TITLE = "Spring"
DESCRIPTION = "Block suspended from a static anchor by a soft stretch constraint."
REPORT_FRAMES = 180
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.viewer.set_camera(pos=wp.vec3(16.13996429, -16.13996429, 15.41640786), pitch=-18.0, yaw=135.0)
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "fov"):
            self.viewer.camera.fov = 55.0
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "look_at"):
            self.viewer.camera.look_at(wp.vec3(0.0, 0.0, 8.0))
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        anchor = builder.add_body(xform=_xform((0.0, 0.0, 14.0)), is_kinematic=True, label="anchor")
        builder.add_shape_box(anchor, hx=0.5, hy=0.5, hz=0.5, cfg=self._shape_cfg(0.0, 0.5), label="anchor_shape")
        block = builder.add_body(xform=_xform((0.0, 0.0, 8.0)), label="block")
        builder.add_shape_box(block, hx=1.0, hy=1.0, hz=1.0, cfg=self._shape_cfg(1.0, 0.5), label="block_shape")
        builder.add_joint_cable(
            parent=anchor,
            child=block,
            parent_xform=_xform((0.0, 0.0, 0.0)),
            child_xform=_xform((0.0, 0.0, 4.0)),
