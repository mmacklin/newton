...
"""AVBD 3D Rope scene recreated with Newton VBD."""
...
TITLE = "Rope"
DESCRIPTION = "Twenty rigid box links connected by hard ball joints."
REPORT_FRAMES = 180
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.viewer.set_camera(pos=wp.vec3(31.28911699, -21.78911699, 19.36902090), pitch=-25.0, yaw=135.0)
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "fov"):
            self.viewer.camera.fov = 55.0
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "look_at"):
            self.viewer.camera.look_at(wp.vec3(9.5, 0.0, 5.0))
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        prev = -1
        for i in range(20):
            body = builder.add_body(xform=_xform((float(i), 0.0, 10.0)), is_kinematic=i == 0, label=f"rope_{i}")
            builder.add_shape_box(
                body,
                hx=0.5,
                hy=0.25,
