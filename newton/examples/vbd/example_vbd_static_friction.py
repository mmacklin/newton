...
DESCRIPTION = "Boxes with friction 0.25 to 0.5 resting on a 30 degree ramp."
REPORT_FRAMES = 180
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.ramp_box_bodies: list[int] = []
        self.ramp_box_initial_positions: list[tuple[float, float, float]] = []
        self.ramp_tangent = np.zeros(3, dtype=np.float64)
        self.ramp_normal = np.zeros(3, dtype=np.float64)
        ...
        self.shadow_distance = 36.0
        self.viewer.set_camera(pos=wp.vec3(18.07082975, -23.07082975, 20.21425742), pitch=-25.0, yaw=135.0)
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "fov"):
            self.viewer.camera.fov = 55.0
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "look_at"):
            self.viewer.camera.look_at(wp.vec3(-5.0, 0.0, 5.0))
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        ramp_position = np.array((0.0, 0.0, 3.0), dtype=np.float64)
        ramp = builder.add_body(
            xform=_xform(tuple(ramp_position), wp.quat_from_axis_angle(wp.vec3(0.0, 1.0, 0.0), angle)),
            is_kinematic=True,
            label="ramp",
        )
        builder.add_shape_box(ramp, hx=20.0, hy=12.0, hz=0.5, cfg=self._shape_cfg(0.0, 1.0), label="ramp_shape")

        self.ramp_tangent = np.array((math.cos(angle), 0.0, -math.sin(angle)), dtype=np.float64)
        self.ramp_normal = np.array((math.sin(angle), 0.0, math.cos(angle)), dtype=np.float64)
        for i in range(11):
            friction = i / 10.0 * 0.25 + 0.25
            pos = ramp_position - 12.0 * self.ramp_tangent + 1.05 * self.ramp_normal
            pos[1] = -10.0 + i * 2.0
            position = (float(pos[0]), float(pos[1]), float(pos[2]))
            body = builder.add_body(xform=_xform(position), label=f"ramp_box_{i}")
            builder.add_shape_box(
                body,
                hx=0.5,
                hy=0.5,
                hz=0.5,
                cfg=self._shape_cfg(1.0, friction),
                label=f"ramp_box_{i}_shape",
            )
            self.ramp_box_bodies.append(body)
            self.ramp_box_initial_positions.append(position)
    ...
    def test_final(self) -> None:
        ...
        for i in range(2):
            body = self.ramp_box_bodies[i]
            initial_pos = np.asarray(self.ramp_box_initial_positions[i], dtype=np.float64)
            pos = body_q[body, 0:3]
            slide = float(np.dot(pos - initial_pos, self.ramp_tangent))
            downhill_speed = float(np.dot(body_qd[body, 0:3], self.ramp_tangent))
        ...
        for i in (9, 10):
            body = self.ramp_box_bodies[i]
            box_normal = _quat_wxyz_to_matrix(body_q[body, 3:7]) @ np.array((0.0, 0.0, 1.0), dtype=np.float64)
            dot = float(np.clip(np.dot(box_normal, self.ramp_normal), -1.0, 1.0))
            tilt = math.degrees(math.acos(dot))
            max_tilt = max(max_tilt, tilt)
            max_speed = max(max_speed, float(np.linalg.norm(body_qd[body, 0:3])))
            max_angular_speed = max(max_angular_speed, float(np.linalg.norm(body_qd[body, 3:6])))

        self.extra_metrics["low_friction_min_slide"] = low_friction_min_slide
        self.extra_metrics["low_friction_min_downhill_speed"] = low_friction_min_downhill_speed
        self.extra_metrics["high_friction_tilt_deg"] = max_tilt
        self.extra_metrics["high_friction_speed"] = max_speed
        self.extra_metrics["high_friction_angular_speed"] = max_angular_speed

        if low_friction_min_slide < 1.5:
            raise AssertionError(
                f"{self.title}: low-friction ramp boxes slid only {low_friction_min_slide:.3f} m; "
                "expected visible downhill slip"
            )
        if low_friction_min_downhill_speed < 0.8:
            raise AssertionError(
                f"{self.title}: low-friction ramp boxes downhill speed only "
                f"{low_friction_min_downhill_speed:.3f} m/s; expected continued sliding"
            )
        if max_tilt > 25.0:
            raise AssertionError(
                f"{self.title}: high-friction ramp boxes tilted {max_tilt:.2f} deg; expected no tumbling"
            )
        if max_speed > 0.75:
            raise AssertionError(
                f"{self.title}: high-friction ramp boxes moving at {max_speed:.3f} m/s; expected sticking"
            )
        if max_angular_speed > 0.75:
            raise AssertionError(
                f"{self.title}: high-friction ramp boxes angular speed {max_angular_speed:.3f} rad/s; "
