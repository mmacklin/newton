...
class Example:
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        self.add_box(builder, (40.0, 24.0, 1.0), 0.0, 1.0, ramp_position, rotation=ramp_quat, label="ramp")
        ...
        for i in range(11):
            ...
            self.add_box(builder, (1.0, 1.0, 1.0), 1.0, friction, pos, label=f"ramp_box_{i}")
    ...
    def _validate_scene(self) -> None:
        ...
        for i in range(2):
            label = f"ramp_box_{i}"
            body = self.body_index(label)
            initial_pos = np.asarray(self.initial_body_positions[body], dtype=np.float64)
            pos = self.body_position(label)
            slide = float(np.dot(pos - initial_pos, ramp_tangent))
            downhill_speed = float(np.dot(self.body_linear_velocity(label), ramp_tangent))
        ...
        for i in (9, 10):
            label = f"ramp_box_{i}"
            box_normal = self.body_local_axis_world(label, (0.0, 0.0, 1.0))
            dot = float(np.clip(np.dot(box_normal, ramp_normal), -1.0, 1.0))
            tilt = math.degrees(math.acos(dot))
            max_tilt = max(max_tilt, tilt)
            max_speed = max(max_speed, self.body_linear_speed(label))
            max_angular_speed = max(max_angular_speed, self.body_angular_speed(label))
