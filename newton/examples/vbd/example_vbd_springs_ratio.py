...
"""AVBD 3D Spring Ratio scene recreated with Newton VBD."""
...
TITLE = "Spring Ratio"
DESCRIPTION = "Alternating soft and stiff stretch constraints between eight links."
REPORT_FRAMES = 180
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.viewer.set_camera(pos=wp.vec3(13.66575096, -29.30629751, 21.50657781), pitch=-18.0, yaw=115.0)
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "fov"):
            self.viewer.camera.fov = 55.0
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "look_at"):
            self.viewer.camera.look_at(wp.vec3(0.0, 0.0, 11.0))
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ground = builder.add_body(xform=_xform((0.0, 0.0, -10.0)), is_kinematic=True, label="ground")
        builder.add_shape_box(ground, hx=50.0, hy=50.0, hz=0.5, cfg=self._shape_cfg(0.0, 0.5), label="ground_shape")
        prev = -1
        for i in range(8):
            x = (i - 3.5) * 3.0
            is_static = i == 0 or i == 7
            body = builder.add_body(xform=_xform((x, 0.0, 12.0)), is_kinematic=is_static, label=f"spring_ratio_{i}")
            builder.add_shape_box(
                body,
                hx=0.5,
                hy=0.375,
                hz=0.375,
                cfg=self._shape_cfg(0.0 if is_static else 1.0, 0.5),
                label=f"spring_ratio_{i}_shape",
            )
            if prev >= 0:
                builder.add_joint_cable(
                    ...
                    child_xform=_xform((-2.5, 0.0, 0.0)),
                    stretch_stiffness=10.0 if i % 2 == 0 else 10000.0,
                    stretch_damping=0.0,
                    bend_stiffness=0.0,
                    bend_damping=0.0,
                    collision_filter_parent=True,
                    label=f"spring_ratio_joint_{i}",
                )
            prev = body
