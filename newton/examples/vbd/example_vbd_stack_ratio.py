...
class Example:
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        self.add_box(builder, (100.0, 100.0, ground_thickness), 0.0, 0.5, (0.0, 0.0, 0.0), label="ground")
        top_z = ground_thickness * 0.5
        size = 1.0
        for i in range(4):
            half = size * 0.5
            center_z = top_z + half
            self.add_box(builder, (size, size, size), 1.0, 0.5, (0.0, 0.0, center_z), label=f"ratio_stack_{i}")
            top_z = center_z + half
            size *= 2.0

    def _validate_scene(self) -> None:
        self.assert_final_speed_below(2.0)
        self.assert_final_angular_speed_below(3.0)
        self.assert_final_xy_radius_below(2.5)
