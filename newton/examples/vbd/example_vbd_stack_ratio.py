...
"""AVBD 3D Stack Ratio scene recreated with Newton VBD."""
...
TITLE = "Stack Ratio"
DESCRIPTION = "Four cubes with side lengths 1, 2, 4, and 8 stacked by increasing mass ratio."
REPORT_FRAMES = 180
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.viewer.set_camera(pos=wp.vec3(19.66853973, -19.66853973, 16.23819780), pitch=-22.0, yaw=135.0)
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "fov"):
            self.viewer.camera.fov = 55.0
        if hasattr(self.viewer, "camera") and hasattr(self.viewer.camera, "look_at"):
            self.viewer.camera.look_at(wp.vec3(0.0, 0.0, 5.0))
    ...
    def _build_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        for i in range(4):
            half = size * 0.5
            center_z = top_z + half
            body = builder.add_body(xform=_xform((0.0, 0.0, center_z)), label=f"ratio_stack_{i}")
            builder.add_shape_box(
                body, hx=half, hy=half, hz=half, cfg=self._shape_cfg(1.0, 0.5), label=f"ratio_stack_{i}_shape"
            )
            top_z = center_z + half
            size *= 2.0
    ...
    def test_final(self) -> None:
        ...
        if self.final_speed > 2.0:
            raise AssertionError(f"{self.title}: final speed {self.final_speed:.3f} exceeds 2.000")
        if self.final_angular_speed > 3.0:
            raise AssertionError(f"{self.title}: final angular speed {self.final_angular_speed:.3f} exceeds 3.000")
        if self.final_xy_radius > 2.5:
            raise AssertionError(f"{self.title}: final xy radius {self.final_xy_radius:.3f} exceeds 2.500")
        if self.final_max_z > 13.0:
            raise AssertionError(
                f"{self.title}: final max z {self.final_max_z:.3f} exceeds 13.000; stack launched upward"
            )
