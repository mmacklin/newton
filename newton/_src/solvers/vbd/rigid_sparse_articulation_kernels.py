...
def _joint_projectors(
    ...
    if jt == JointType.REVOLUTE:
        axis_w = wp.normalize(wp.quat_rotate(parent_anchor_q, joint_axis[qd_start]))
        P_ang = P_ang - wp.outer(axis_w, axis_w)
    elif jt == JointType.D6:
        if ang_count > 0:
            axis_a0 = wp.normalize(wp.quat_rotate(parent_anchor_q, joint_axis[qd_start + lin_count]))
            P_ang = P_ang - wp.outer(axis_a0, axis_a0)
        if ang_count > 1:
            axis_a1 = wp.normalize(wp.quat_rotate(parent_anchor_q, joint_axis[qd_start + lin_count + 1]))
            P_ang = P_ang - wp.outer(axis_a1, axis_a1)
        if ang_count > 2:
            axis_a2 = wp.normalize(wp.quat_rotate(parent_anchor_q, joint_axis[qd_start + lin_count + 2]))
...
def solve_articulation_sparse_cpu(
    ...
    for local_body in range(body_start, body_end):
        ...
        if body_inv_mass[body] == 0.0:
            ...
            rhs[local_body] = _vec6_from_parts(f_lin, tau_world)
    ...
    for local_body in range(body_start, body_end):
        ...
        if body_inv_mass[body] == 0.0:
            ...
            dx = delta[local_body] * update_relaxation
