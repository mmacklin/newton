...
<p>For an equal-workload backend comparison, the DVI row holds the policy timestep and contact envelope fixed rather than adopting the smaller timestep used by the standalone Kamino DVI example. It uses Moreau integration, sparse dynamics and Jacobians, CR with nine iterations, four block iterations, and two contact iterations.</p>
...
<p>Sparse VBD completes the rollout with mean forward-speed error {_fmt(abs(dr_policy_sparse["forward_velocity_error_mps"]["mean"]), 3)} m/s and {_fmt(dr_policy_sparse["closure_error_um"]["rms"], 1)} µm closure RMS. Both Kamino backends complete. DVI lowers closure RMS from {_fmt(dr_policy_kamino["closure_error_um"]["rms"], 1)} µm to {_fmt(dr_policy_kamino_dvi["closure_error_um"]["rms"], 1)} µm relative to PADMM, but follows a substantially different trajectory, including {_fmt(abs(dr_policy_kamino_dvi["lateral_displacement_m"]), 3)} m lateral drift. Local i32 reaches closure error comparable to sparse i8 before falling, so geometric closure is necessary but not sufficient for policy transfer. The no-armature sparse ablation is even more direct: it holds closure to {_fmt(dr_policy_sparse_no_armature["closure_error_um"]["rms"], 1)} µm RMS yet falls at {dr_policy_sparse_no_armature["fall_time_s"]:.2f} s because the controlled dynamics no longer match training.</p>
...
{_video("dr_legs_policy_kamino_dvi.mp4", "Kamino DVI policy rollout", "Matched 250 Hz policy workload with the optimized sparse DVI backend.", anchor="kamino-dvi-dr-legs-policy")}
