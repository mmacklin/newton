...
<h3 class="analysis-heading">Performance</h3>
<table><thead><tr><th>Backend</th><th>Solver</th><th>Run status</th><th>p50 solver [ms]</th><th>p50 collision [ms]</th><th>Combined p50 [ms]</th><th>Speedup vs Kamino</th></tr></thead><tbody>
...
<tr><td>CUDA MIG</td><td>Kamino</td><td>complete</td><td>{_fmt(dr_cuda_kamino["p50_solver_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_cuda_kamino["p50_collision_us"] / 1.0e3, 3)}</td><td>{_fmt((dr_cuda_kamino["p50_solver_us"] + dr_cuda_kamino["p50_collision_us"]) / 1.0e3, 3)}</td><td>1.00x</td></tr>
<tr><td>CUDA MIG</td><td>VBD local</td><td>not run after CPU failure</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>
<tr><td>CUDA MIG</td><td><strong>VBD sparse direct, i8</strong></td><td>complete</td><td><strong>{_fmt(dr_cuda_sparse["p50_solver_us"] / 1.0e3, 3)}</strong></td><td>{_fmt(dr_cuda_sparse["p50_collision_us"] / 1.0e3, 3)}</td><td><strong>{_fmt((dr_cuda_sparse["p50_solver_us"] + dr_cuda_sparse["p50_collision_us"]) / 1.0e3, 3)}</strong></td><td><strong>{_fmt((dr_cuda_kamino["p50_solver_us"] + dr_cuda_kamino["p50_collision_us"]) / (dr_cuda_sparse["p50_solver_us"] + dr_cuda_sparse["p50_collision_us"]), 2)}x</strong></td></tr>
</tbody></table>
<p class="note">DR Legs timings begin after 40 warm-up frames. Collision and solver are synchronized and timed separately; “combined” is the sum of their medians, and speedup uses that combined value. * Local VBD timing covers only its finite prefix and is not a successful-throughput result.</p>
<div class="media-grid">
