...
<p>Sparse VBD completes the rollout with mean forward-speed error {_fmt(abs(dr_policy_sparse["forward_velocity_error_mps"]["mean"]), 3)} m/s and {_fmt(dr_policy_sparse["closure_error_um"]["rms"], 1)} µm closure RMS. Both Kamino backends complete. The tuned DVI trajectory has {_fmt(abs(dr_policy_kamino_dvi["lateral_displacement_m"]), 3)} m lateral drift and closes the loops to {_fmt(dr_policy_kamino_dvi["closure_error_um"]["rms"], 1)} µm RMS. Local i32 reaches closure error comparable to sparse i8 before falling, so geometric closure is necessary but not sufficient for policy transfer. The no-armature sparse ablation is even more direct: it holds closure to {_fmt(dr_policy_sparse_no_armature["closure_error_um"]["rms"], 1)} µm RMS yet falls at {dr_policy_sparse_no_armature["fall_time_s"]:.2f} s because the controlled dynamics no longer match training.</p>
