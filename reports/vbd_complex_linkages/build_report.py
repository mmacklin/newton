...
def _video(name: str, title: str, text: str, anchor: str | None = None) -> str:
    stem = name.removesuffix(".mp4")
    heading_id = f' id="{html.escape(anchor)}"' if anchor else ""
    heading = html.escape(title)
    if anchor:
        heading = f'<a class="section-link" href="#{html.escape(anchor)}">{heading}</a>'
    return f"""
    <article class="media">
      <h3{heading_id}>{heading}</h3>
...
def main() -> None:
    ...
    for name in (
        ...
        "dr_legs_dvi_contact_sweep.json",
    ...
    dr_policy_kamino_dvi = dr_policy_rows["kamino_dvi"]
    ...
    dr_policy_sparse_isotropic = dr_policy_rows["sparse_isotropic_armature_i8"]
...
.section-link:hover::after,.section-link:focus::after{{color:var(--accent);opacity:1}}
...
<h2 id="summary"><a class="section-link" href="#summary">Summary</a></h2>
...
<h2 id="primal-maximal-formulation"><a class="section-link" href="#primal-maximal-formulation">Primal maximal-coordinate formulation</a></h2>
...
<h2 id="joint-armature"><a class="section-link" href="#joint-armature">Coupled revolute-joint armature</a></h2>
<p>A revolute armature value \(a_j\) represents rotor inertia in the relative joint coordinate. If parent and child angular velocities are \(\boldsymbol{{\omega}}_p\) and \(\boldsymbol{{\omega}}_c\), its continuous kinetic energy is</p>
\[
T_{{\mathrm{{arm}},j}}=\frac12 a_j\left[\mathbf{{u}}^T(\boldsymbol{{\omega}}_c-\boldsymbol{{\omega}}_p)\right]^2.
\]
<p>VBD evaluates the relative rotation error \(e_j\) between the current parent-child joint transform and its forward-integrated inertial target. The added positional inertial potential is</p>
\[
E_{{\mathrm{{arm}},j}}=\frac{{a_j}}{{2\Delta t^2}}e_j^2.
\]
<div class="formula-grid">
  <div>
    <h3>Coupled physical contribution</h3>
    <p>With \(h=a_j/\Delta t^2\) and world angular Jacobian \(\mathbf{{j}}\), Gauss-Newton assembly adds</p>
    \[
    H_{{\mathrm{{arm}}}}=h\begin{{bmatrix}}\mathbf{{j}}\mathbf{{j}}^T&amp;-\mathbf{{j}}\mathbf{{j}}^T\\-\mathbf{{j}}\mathbf{{j}}^T&amp;\mathbf{{j}}\mathbf{{j}}^T\end{{bmatrix}}.
    \]
    <p>Equal parent and child rotation lies in its nullspace; armature resists only relative motion and produces equal-and-opposite reactions.</p>
  </div>
  <div>
    <h3>Previous child-body approximation</h3>
    <p>Adding \(a_j I_3\) to only the child inertia creates a body-diagonal term with no parent or cross block. It penalizes absolute child rotation, including common parent-child motion, and adds inertia around unrelated axes.</p>
    \[
    H_{{\mathrm{{child}}}}=h\begin{{bmatrix}}0&amp;0\\0&amp;I_3\end{{bmatrix}}.
    \]
    <p>It is retained only as an explicit policy-transfer ablation.</p>
  </div>
</div>
<p class="note">The nonlinear SO(3) residual uses a positive-semidefinite Gauss-Newton Hessian; the geometric second derivative is omitted. A focused two-body test matches the small-angle analytic solution to 6.2e-12 rad and preserves angular momentum to 5.8e-12. See the <a href="https://reports.mmacklin.com/vbd-joint-armature/">standalone armature report</a> for the derivation and diagrams.</p>

<h2 id="sparse-structure-contact"><a class="section-link" href="#sparse-structure-contact">Sparse structure and contact treatment</a></h2>
<figure>
...
<h2 id="implementation"><a class="section-link" href="#implementation">Implementation and code</a></h2>
<div class="code-grid">
  <div>
...
<h2 id="synthetic-convergence"><a class="section-link" href="#synthetic-convergence">Synthetic convergence evidence</a></h2>
...
<h2 id="visual-validations"><a class="section-link" href="#visual-validations">Public visual validations</a></h2>
...
<p>The cable is reset to the same perturbed state for each residual measurement. CPU latency is measured separately as median fixed-buffer CPU-graph replay time over 50 samples. The plot therefore compares achieved joint error against measured solver cost rather than assuming equal iterations imply equal work.</p>
...
<h2 id="evaluation-methods"><a class="section-link" href="#evaluation-methods">Evaluation methods</a></h2>
...
<h2 id="three-pushrod-robot-foot"><a class="section-link" href="#three-pushrod-robot-foot">Three-pushrod robot foot</a></h2>
...
<table><thead><tr><th>Solver</th><th>Timing path</th><th>p50 substep [ms]</th><th>p90 substep [ms]</th><th>Speedup vs Kamino</th></tr></thead><tbody>
<tr><td>Kamino</td><td>dispatch</td><td>{_fmt(foot_rows["kamino"]["p50_step_us"] / 1.0e3, 3)}</td><td>{_fmt(foot_rows["kamino"]["p90_step_us"] / 1.0e3, 3)}</td><td>1.00x</td></tr>
<tr><td>VBD local, i8</td><td>CPU graph</td><td>{_fmt(foot_local["p50_step_us"] / 1.0e3, 3)}</td><td>{_fmt(foot_local["p90_step_us"] / 1.0e3, 3)}</td><td>{_fmt(foot_rows["kamino"]["p50_step_us"] / foot_local["p50_step_us"], 2)}x</td></tr>
<tr><td><strong>VBD sparse direct, i8</strong></td><td>CPU graph</td><td><strong>{_fmt(foot_sparse["p50_step_us"] / 1.0e3, 3)}</strong></td><td>{_fmt(foot_sparse["p90_step_us"] / 1.0e3, 3)}</td><td><strong>{_fmt(foot_rows["kamino"]["p50_step_us"] / foot_sparse["p50_step_us"], 2)}x</strong></td></tr>
</tbody></table>
<p class="note">VBD timings replay the complete fixed-buffer solver step through CPU graphs. Kamino remains dispatched because its host-controlled PADMM decisions are not graph-replay safe; Kamino-relative speedups therefore include its dispatch overhead.</p>
...
<h2 id="g1-closed-loop-ankle"><a class="section-link" href="#g1-closed-loop-ankle">G1 closed-loop ankle</a></h2>
...
<h2 id="unitree-h2"><a class="section-link" href="#unitree-h2">Unitree H2: six closed-loop rods</a></h2>
...
<h2 id="dr-legs-ground-contact"><a class="section-link" href="#dr-legs-ground-contact">DR Legs with ground contact</a></h2>
...
<h3 class="analysis-heading">Performance</h3>
<table><thead><tr><th>Backend</th><th>Solver</th><th>Timing path</th><th>Run status</th><th>p50 solver [ms]</th><th>p50 collision [ms]</th><th>End-to-end p50 [ms]</th><th>Speedup vs Kamino</th></tr></thead><tbody>
<tr><td>CPU</td><td>Kamino</td><td>dispatch</td><td>complete</td><td>{_fmt(dr_free_kamino["p50_solver_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_free_kamino["p50_collision_us"] / 1.0e3, 3)}</td><td>{_fmt((dr_free_kamino["p50_solver_us"] + dr_free_kamino["p50_collision_us"]) / 1.0e3, 3)}</td><td>1.00x</td></tr>
<tr><td>CPU</td><td>VBD local, i8</td><td>solver graph</td><td>fails at {dr_free_local["failure_substep"]}</td><td>{_fmt(dr_free_local["p50_solver_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_free_local["p50_collision_us"] / 1.0e3, 3)}</td><td>{_fmt((dr_free_local["p50_solver_us"] + dr_free_local["p50_collision_us"]) / 1.0e3, 3)}</td><td>{_fmt((dr_free_kamino["p50_solver_us"] + dr_free_kamino["p50_collision_us"]) / (dr_free_local["p50_solver_us"] + dr_free_local["p50_collision_us"]), 2)}x*</td></tr>
<tr><td>CPU</td><td><strong>VBD sparse direct, i8</strong></td><td>solver graph</td><td>complete</td><td><strong>{_fmt(dr_free_sparse["p50_solver_us"] / 1.0e3, 3)}</strong></td><td>{_fmt(dr_free_sparse["p50_collision_us"] / 1.0e3, 3)}</td><td><strong>{_fmt((dr_free_sparse["p50_solver_us"] + dr_free_sparse["p50_collision_us"]) / 1.0e3, 3)}</strong></td><td><strong>{_fmt((dr_free_kamino["p50_solver_us"] + dr_free_kamino["p50_collision_us"]) / (dr_free_sparse["p50_solver_us"] + dr_free_sparse["p50_collision_us"]), 2)}x</strong></td></tr>
<tr><td>CUDA MIG</td><td>Kamino</td><td>end-to-end graph</td><td>complete</td><td>included</td><td>included</td><td>{_fmt(dr_cuda_kamino["p50_graph_step_us"] / 1.0e3, 3)}</td><td>1.00x</td></tr>
<tr><td>CUDA MIG</td><td>VBD local</td><td>-</td><td>not run after CPU failure</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>
<tr><td>CUDA MIG</td><td><strong>VBD sparse direct, i8</strong></td><td>end-to-end graph</td><td>complete</td><td>included</td><td>included</td><td><strong>{_fmt(dr_cuda_sparse["p50_graph_step_us"] / 1.0e3, 3)}</strong></td><td><strong>{_fmt(dr_cuda_kamino["p50_graph_step_us"] / dr_cuda_sparse["p50_graph_step_us"], 2)}x</strong></td></tr>
</tbody></table>
<p class="note">DR Legs timings begin after 40 warm-up frames. CPU collision and solver are synchronized and timed separately; CPU end-to-end p50 is the sum of their medians. VBD solver values are CPU-graph replay times, while Kamino CPU includes normal dispatch because graph capture freezes its host-controlled PADMM decisions and does not produce a valid trajectory. Each CUDA value is one synchronized replay of an end-to-end ping-pong graph, so collision and solver are included rather than timed separately. CPU speedup versus Kamino therefore includes Kamino dispatch overhead. * Local VBD timing covers only its finite prefix and is not a successful-throughput result.</p>
<div class="media-grid">
{_video("dr_legs_kamino_free_ankle_cpu.mp4", "Kamino free ankle", "The two ankle drives are disabled while the ten hip and linkage drives remain active.", anchor="kamino-dr-legs-free-ankle")}
...
<h2 id="dr-legs-policy"><a class="section-link" href="#dr-legs-policy">Trained DR Legs walking policy</a></h2>
<p>This test runs the trained policy distributed with Newton's Disney Research assets. It preserves the SDK policy contract: a 94-dimensional observation, 12 position actions scaled by 0.4, <code>kp=15</code>, <code>kd=0.6</code>, a 250 Hz physics rate (<code>dt=0.004 s</code>), and one policy inference every five substeps. The robot stands for 1 s, then receives a 0.2 m/s forward command for 7 s. Kamino PADMM, Kamino DVI, and both VBD modes use the same imported 31-body model and Newton contact pipeline. The <a href="{GITHUB_BLOB}/reports/vbd_complex_linkages/bench_dr_legs_policy.py">benchmark harness</a> reconstructs the exact observation ordering and checkpoint normalizer used by the SDK example; the <a href="{GITHUB_BLOB}/reports/vbd_complex_linkages/render_dr_legs_policy.py">ViewerGL renderer</a> records the same execution path.</p>
<p>For an equal-workload backend comparison, the DVI row holds the policy timestep and contact envelope fixed rather than adopting the smaller timestep used by the standalone Kamino DVI example. It uses Moreau integration, sparse dynamics and Jacobians, CR with nine iterations, four block iterations, six contact iterations, the 3&times;3 contact block preconditioner, and penetration stabilization <code>gamma=0.1</code>. These contact parameters were selected by the documented <a href="dr_legs_dvi_contact_sweep.json">DVI sweep</a>.</p>
<div class="equation-note">
<strong>Armature treatment.</strong> The policy was trained with <code>0.01 kg·m²</code> armature on every actuator. Sparse VBD now assembles this as coupled relative-coordinate inertia, adding parent and child rank-one angular blocks and their negative cross block. The previous isotropic child-body approximation and a no-armature run are retained as ablations. Local VBD still uses the child-body approximation because it cannot retain parent-child cross blocks.
</div>
...
<tr><td>Kamino PADMM</td><td>native joint armature</td><td>completes 8 s</td><td>{_fmt(dr_policy_kamino["forward_velocity_mps"]["mean"], 3)}</td><td>{_fmt(dr_policy_kamino["forward_displacement_m"], 3)}</td><td>{_fmt(abs(dr_policy_kamino["lateral_displacement_m"]), 3)}</td><td>{_fmt(dr_policy_kamino["height_m"]["min"], 3)}&ndash;{_fmt(dr_policy_kamino["height_m"]["max"], 3)}</td><td>{_fmt(dr_policy_kamino["tilt_deg"]["rms"], 1)} / {_fmt(dr_policy_kamino["tilt_deg"]["max"], 1)}</td><td>{_fmt(dr_policy_kamino["closure_error_um"]["rms"], 1)}</td></tr>
<tr><td>Kamino DVI</td><td>native joint armature</td><td>completes 8 s</td><td>{_fmt(dr_policy_kamino_dvi["forward_velocity_mps"]["mean"], 3)}</td><td>{_fmt(dr_policy_kamino_dvi["forward_displacement_m"], 3)}</td><td>{_fmt(abs(dr_policy_kamino_dvi["lateral_displacement_m"]), 3)}</td><td>{_fmt(dr_policy_kamino_dvi["height_m"]["min"], 3)}&ndash;{_fmt(dr_policy_kamino_dvi["height_m"]["max"], 3)}</td><td>{_fmt(dr_policy_kamino_dvi["tilt_deg"]["rms"], 1)} / {_fmt(dr_policy_kamino_dvi["tilt_deg"]["max"], 1)}</td><td>{_fmt(dr_policy_kamino_dvi["closure_error_um"]["rms"], 1)}</td></tr>
...
<tr><td>VBD sparse direct, i8</td><td>disabled ablation</td><td>falls at {dr_policy_sparse_no_armature["fall_time_s"]:.2f} s</td><td>-</td><td>{_fmt(dr_policy_sparse_no_armature["forward_displacement_m"], 3)}</td><td>{_fmt(abs(dr_policy_sparse_no_armature["lateral_displacement_m"]), 3)}</td><td>{_fmt(dr_policy_sparse_no_armature["height_m"]["min"], 3)}&ndash;{_fmt(dr_policy_sparse_no_armature["height_m"]["max"], 3)}</td><td>{_fmt(dr_policy_sparse_no_armature["tilt_deg"]["rms"], 1)} / {_fmt(dr_policy_sparse_no_armature["tilt_deg"]["max"], 1)}</td><td>{_fmt(dr_policy_sparse_no_armature["closure_error_um"]["rms"], 1)}</td></tr>
<tr><td>VBD sparse direct, i8</td><td>isotropic child body</td><td>completes 8 s</td><td>{_fmt(dr_policy_sparse_isotropic["forward_velocity_mps"]["mean"], 3)}</td><td>{_fmt(dr_policy_sparse_isotropic["forward_displacement_m"], 3)}</td><td>{_fmt(abs(dr_policy_sparse_isotropic["lateral_displacement_m"]), 3)}</td><td>{_fmt(dr_policy_sparse_isotropic["height_m"]["min"], 3)}&ndash;{_fmt(dr_policy_sparse_isotropic["height_m"]["max"], 3)}</td><td>{_fmt(dr_policy_sparse_isotropic["tilt_deg"]["rms"], 1)} / {_fmt(dr_policy_sparse_isotropic["tilt_deg"]["max"], 1)}</td><td>{_fmt(dr_policy_sparse_isotropic["closure_error_um"]["rms"], 1)}</td></tr>
<tr><td><strong>VBD sparse direct, i8</strong></td><td><strong>coupled joint armature</strong></td><td><strong>completes 8 s</strong></td><td><strong>{_fmt(dr_policy_sparse["forward_velocity_mps"]["mean"], 3)}</strong></td><td><strong>{_fmt(dr_policy_sparse["forward_displacement_m"], 3)}</strong></td><td><strong>{_fmt(abs(dr_policy_sparse["lateral_displacement_m"]), 3)}</strong></td><td>{_fmt(dr_policy_sparse["height_m"]["min"], 3)}&ndash;{_fmt(dr_policy_sparse["height_m"]["max"], 3)}</td><td>{_fmt(dr_policy_sparse["tilt_deg"]["rms"], 1)} / {_fmt(dr_policy_sparse["tilt_deg"]["max"], 1)}</td><td><strong>{_fmt(dr_policy_sparse["closure_error_um"]["rms"], 1)}</strong></td></tr>
</tbody></table>
<p>Coupled-armature sparse VBD completes the rollout with mean forward-speed error {_fmt(abs(dr_policy_sparse["forward_velocity_error_mps"]["mean"]), 3)} m/s and {_fmt(dr_policy_sparse["closure_error_um"]["rms"], 1)} µm closure RMS. Relative to the isotropic approximation, pelvis tilt RMS falls from {_fmt(dr_policy_sparse_isotropic["tilt_deg"]["rms"], 2)}&deg; to {_fmt(dr_policy_sparse["tilt_deg"]["rms"], 2)}&deg;, lateral drift falls from {_fmt(abs(dr_policy_sparse_isotropic["lateral_displacement_m"]), 3)} m to {_fmt(abs(dr_policy_sparse["lateral_displacement_m"]), 3)} m, and actuator target-error RMS falls from {_fmt(dr_policy_sparse_isotropic["actuated_target_error_rad"]["rms"], 3)} rad to {_fmt(dr_policy_sparse["actuated_target_error_rad"]["rms"], 3)} rad. Both Kamino backends complete. The no-armature sparse ablation holds closure to {_fmt(dr_policy_sparse_no_armature["closure_error_um"]["rms"], 1)} µm RMS yet falls at {dr_policy_sparse_no_armature["fall_time_s"]:.2f} s because the controlled dynamics no longer match training.</p>
<h3 class="analysis-heading">Ground contact quality</h3>
<table><thead><tr><th>CPU solver</th><th>Maximum walking penetration [mm]</th><th>Mean frame-maximum penetration [mm]</th></tr></thead><tbody>
<tr><td>Kamino PADMM</td><td>{_fmt(dr_policy_kamino["walk_ground_frame_max_penetration_mm"]["max"], 2)}</td><td>{_fmt(dr_policy_kamino["walk_ground_frame_max_penetration_mm"]["mean"], 2)}</td></tr>
<tr><td>Kamino DVI</td><td>{_fmt(dr_policy_kamino_dvi["walk_ground_frame_max_penetration_mm"]["max"], 2)}</td><td>{_fmt(dr_policy_kamino_dvi["walk_ground_frame_max_penetration_mm"]["mean"], 2)}</td></tr>
<tr><td><strong>VBD sparse direct, i8</strong></td><td><strong>{_fmt(dr_policy_sparse["walk_ground_frame_max_penetration_mm"]["max"], 2)}</strong></td><td><strong>{_fmt(dr_policy_sparse["walk_ground_frame_max_penetration_mm"]["mean"], 2)}</strong></td></tr>
</tbody></table>
<p class="note">Penetration is recomputed from signed collision-shape separation once per 20 ms policy frame and is excluded from the timing values. The original DVI settings used two scalar contact iterations and <code>gamma=0.015</code>; they reached 35.6 mm peak walking penetration. The tuned row above fixes the misleading visual result at a measured CPU cost.</p>
<h3 class="analysis-heading">Diagnostic CPU cost</h3>
<table><thead><tr><th>Solver</th><th>Run status</th><th>p50 solver [ms]</th><th>p50 collision [ms]</th><th>p50 physics substep [ms]</th><th>Speedup vs PADMM</th></tr></thead><tbody>
<tr><td>Kamino PADMM</td><td>complete</td><td>{_fmt(dr_policy_kamino["solver_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_kamino["collision_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_kamino["step_p50_us"] / 1.0e3, 3)}</td><td>1.00x</td></tr>
<tr><td>Kamino DVI</td><td>complete</td><td>{_fmt(dr_policy_kamino_dvi["solver_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_kamino_dvi["collision_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_kamino_dvi["step_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_kamino["step_p50_us"] / dr_policy_kamino_dvi["step_p50_us"], 1)}x</td></tr>
...
<tr><td>VBD sparse, isotropic ablation, i8</td><td>complete</td><td>{_fmt(dr_policy_sparse_isotropic["solver_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_sparse_isotropic["collision_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_sparse_isotropic["step_p50_us"] / 1.0e3, 3)}</td><td>{_fmt(dr_policy_kamino["step_p50_us"] / dr_policy_sparse_isotropic["step_p50_us"], 1)}x</td></tr>
...
{_video("dr_legs_policy_sparse_isotropic_armature_i8.mp4", "VBD sparse direct, isotropic armature ablation", "The previous child-body approximation completes but produces visibly looser root motion and substantially more lateral drift.")}
{_video("dr_legs_policy_sparse_i8.mp4", "VBD sparse direct, coupled joint armature", "The corrected parent-child rank-one inertia coupling completes the full trained-policy rollout with motion close to the Kamino references.")}
</div>

<h2 id="numerical-validation"><a class="section-link" href="#numerical-validation">Numerical validation</a></h2>
...
<h2 id="interpretation"><a class="section-link" href="#interpretation">Interpretation</a></h2>
...
<p class="note">Reproducible data: <a href="robot_foot_compatible_results.json">compatible robot foot</a>, <a href="robot_foot_geometry_diagnostic.json">foot geometry check</a>, <a href="g1_ankle_results.json">G1 ankle</a>, <a href="h2_loop_results.json">Unitree H2 loops</a>, <a href="dr_legs_free_ankle_results.json">DR Legs CPU</a>, <a href="dr_legs_free_ankle_cuda_results.json">DR Legs CUDA</a>, <a href="dr_legs_policy_results.json">DR Legs policy</a>, <a href="dr_legs_dvi_contact_sweep.json">DVI contact sweep</a>, <a href="kamino_upstream_optimization_results.json">Kamino optimization comparison</a>, <a href="dr_legs_matrix_diagnostic.json">numerical checks</a>, and <a href="visual_validation_results.json">public visual validations</a>.</p>
</main></body></html>"""
